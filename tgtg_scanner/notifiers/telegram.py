...
class Telegram(Notifier):
        ...
        if self.enabled:
            ...
            try:
                ...
                self.application = Application.builder().token(
                    self.token).arbitrary_callback_data(True).build()
    ...
    def _handlers(self):
        return [
            ...
            CommandHandler(
                "reservations", self._cancel_reservations_menu),
    ...
    async def _remove_favorites(self,
                                update: Update,
                                context: CallbackContext) -> None:
        if not context.args:
            await update.message.reply_text(
