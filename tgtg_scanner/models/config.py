...
DEFAULT_CONFIG = {
    ...
    "console": {
        "enabled": False,
        "body": "${{scanned_on}} ${{display_name}} - " "new amount: ${{items_available}}",
        "cron": Cron("* * * * *"),
    },
    "push_safer": {
        "enabled": False,
    ...
    "ntfy": {
        ...
        "message": "${{display_name}} - New Amount: " "${{items_available}} - ${{link}}",
        "body": None,
        "priority": "default",
        "tags": "shopping,tgtg",
        "click": "${{link}}",
    ...
    "script": {"enabled": False, "command": "", "cron": Cron("* * * * *")},
}
...
class Config:
    ...
    def __init__(self, file: Union[str, None] = None):
        ...
        if self.file is not None:
            if not self.file.is_file():
                raise ConfigurationError(f"Configuration file '{self.file.absolute()}' " "is not a file!")
            self._read_ini()
    ...
    def _ini_get_dict(self, config: configparser.ConfigParser, section: str, key: str, attr: str) -> None:
        if section in config:
            value = config[section].get(key, None)
            if value:
    ...
    def _read_ini(self) -> None:
        ...
        try:
            ...
            self._ini_get_boolean(config, "TELEGRAM", "disableCommands", "telegram.disable_commands")
            ...
            self._ini_get_boolean(config, "SCRIPT", "enabled", "script.enabled")
            ...
            self._ini_get(config, "LOCATION", "Address", "location.origin_address")
            self._ini_get(config, "LOCATION", "Google_Maps_API_Key", "location.gmaps_api_key")
    ...
    def _read_env(self) -> None:
        try:
            ...
            self._env_get_boolean("TELEGRAM_DISABLE_COMMANDS", "telegram.disable_commands")
            ...
            self._env_get("LOCATION_GOOGLE_MAPS_API_KEY", "location.gmaps_api_key")
