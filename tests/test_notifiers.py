...
def test_ifttt(test_item: Item, default_config: Config):
    ...
    default_config._setattr(
        "ifttt.body", '{"value1": "${{display_name}}", "value2": ${{items_available}}, "value3": "https://share.toogoodtogo.com/item/${{item_id}}"}')
    responses.add(
        responses.POST,
        f"https://maker.ifttt.com/trigger/{default_config.ifttt.get('event')}/with/key/{default_config.ifttt.get('key')}",
    ...
    assert responses.calls[0].request.body.decode(
        "utf-8") == f'{{"value1": "{test_item.display_name}", "value2": {test_item.items_available}, "value3": "https://share.toogoodtogo.com/item/{test_item.item_id}"}}'
